...
async fn missing_key_is_not_configured() {
    ...
    match adapter.fetch().await {
        ProviderStatus::NotConfigured { hint } => {
            assert!(hint.contains("EXA_API_KEY"), "{hint}");
...
#[tokio::test]
async fn key_without_remaining_get_is_unsupported_not_usage_poll() {
    let home = std::env::temp_dir().join(format!(
        "tb-exa-key-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
    ...
    match adapter.fetch().await {
        ProviderStatus::Unsupported { reason } => {
            assert!(reason.contains("dashboard.exa.ai/billing"), "{reason}");
        }
        other => panic!("{other:?}"),
    }
}
