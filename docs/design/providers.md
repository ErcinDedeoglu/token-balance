...
| **Grok SuperGrok** | unofficial: `~/.grok/auth.json` then `cli-chat-proxy.grok.com/v1/billing?format=credits` | Grok CLI login only | weekly % + extra credits | No public remaining API. A weekly `currentPeriod` with omitted `creditUsagePercent` is 0% used (fresh period; CLI logs the same null). Prepaid-only or monthly spend JSON stays `Error`. Schema change → `Error`, not panic. |
