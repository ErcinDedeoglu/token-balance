...
| **Exa** | Remaining is the Billing dashboard (`https://dashboard.exa.ai/billing`). No documented remaining-balance GET. `GET https://admin-api.exa.ai/team-management/api-keys/{id}/usage` returns `total_cost_usd` (**spend**, not remaining). | `EXA_API_KEY` or a key file pointer (`vendor = "exa"`). | `LedgerKind::PrepaidWallet`. Spend-only JSON and `NO_MORE_CREDITS` → `Error`, never 5h/weekly %. Missing secret → `NotConfigured`. Live remaining poll is out until a remaining source exists. | Do not paint `/usage` as leftover credits. Do not scrape dashboard cookies. Do not poll `/search` (burns credits). Do not use per-key `budgetCents` as team remaining. |
