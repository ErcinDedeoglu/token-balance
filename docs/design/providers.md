...
| **Exa** | Unofficial `GET https://dashboard.exa.ai/api/get-credits`. `orbCreditsInCents / 100` → USD. Vercel checkpoints CLI HTTP (429); if `chrome-mcp` sees a `dashboard.exa.ai` tab, fetch in-page instead. Official `/usage` is spend. `EXA_API_KEY` cannot read remaining. | User-owned `exa.json` cookie; keep billing tab logged in for the Chrome fallback. Never commit cookies. | `LedgerKind::PrepaidWallet`. Teal `$`. **No 5h/weekly %.** | Do not paint `/usage` as remaining. Do not poll `/search`. |
