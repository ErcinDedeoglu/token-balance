...
| **Exa** | Unofficial dashboard: `GET https://dashboard.exa.ai/api/get-credits` (session cookie). `orbCreditsInCents / 100` → USD remaining. Official `GET https://admin-api.exa.ai/team-management/api-keys/{id}/usage` is `total_cost_usd` (**spend**, not remaining). `EXA_API_KEY` cannot read remaining. | User-owned `exa.json`: `{ "cookie": "next-auth.session-token=…" }` via `credentials`. Never auto-scrape a browser. | `LedgerKind::PrepaidWallet`. Teal `$` remaining. **No 5h/weekly %.** Plan text `Pay as you go`. Spend-only JSON and `NO_MORE_CREDITS` → `Error`. | Do not paint `/usage` as leftover credits. Do not commit dashboard cookies. Do not poll `/search` (burns credits). Do not use per-key `budgetCents` as team remaining. |
