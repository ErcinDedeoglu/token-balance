...
The **default scan surface** is a remaining comparison table (`t view:table`): one packed row per account, columns 5h remaining / weekly remaining / mo remaining / weekly-or-monthly reset / extra. Leftover width sits after extra, not between name and 5h. Plan-remaining rows sit above `PrepaidWallet` rows. 7-row Ember Ledger cards are the optional 7-row t toggle view (`t view:cards`). Launch always starts in table (not persisted).
...
2. **Table (default):** one row per **account** (not per vendor). Glyph + unique label (name column sized to the clipped label, not leftover width); then packed **5h / wk / mo / reset / extra** with one-column gutters; leftover width after extra. 5h cell is the session-class window (`duration_mins <= 360` or `FiveHour`) or em dash; wk cell is `Weekly` or em dash; **mo remaining** is the non-session non-weekly `Other` window (min remaining if several) or em dash; reset is weekly or monthly countdown (lowest remaining among those; never 5h when a week/month exists); session-only plans (Muse Everyday) fall back to the 5h clock; prepaid: `no reset`; extra is teal remaining. Claude extra-only enterprise `extra_usage` is monthly `Other("mo")` with `resets_at` when present, else next UTC month start. Prepaid rows have teal `$` / `cr`, no remaining-percent gauge. Unsigned / unsupported never numeric `0%`. Zero accounts: hint naming `.config/token-balance/accounts.toml` (no 5h/wk/mo header). Table `j`/`k` move one row; `h`/`l` no-op. Sort by **risk** (below); prepaid sinks after plan inside the available group.
