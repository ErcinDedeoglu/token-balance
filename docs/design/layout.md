...
2. **Table (default):** one row per **account** (not per vendor). Glyph + unique label (name column sized to the clipped label, not leftover width); then packed **5h / wk / mo / reset / extra** with one-column gutters; leftover width after extra. 5h cell is the session-class window (`duration_mins <= 360` or `FiveHour`) or em dash; wk cell is `Weekly` or em dash; **mo remaining** is the non-session non-weekly `Other` window (min remaining if several) or em dash; reset is soonest `resets_at` countdown (prepaid: `no reset`); extra is teal remaining. Prepaid rows have teal `$` / `cr`, no remaining-percent gauge. Unsigned / unsupported never numeric `0%`. Zero accounts: hint naming `.config/token-balance/accounts.toml` (no 5h/wk/mo header). Table `j`/`k` move one row; `h`/`l` no-op. Sort by **risk** (below); prepaid sinks after plan inside the available group.
